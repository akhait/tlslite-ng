...
class TLSConnection(TLSRecordLayer):
    ...
    def _handshakeClientAsyncHelper(self, srpParams, certParams, anonParams,
        ...
        self.version = (3, 1) if settings.maxVersion > (3, 3) \
                       else settings.maxVersion
    ...
    def _clientSendClientHello(self, settings, session, srpUsername,
        ...
        sent_version = settings.maxVersion if settings.maxVersion <= (3, 3)\
                       else (3, 3)
    ...
    def _clientTLS13Handshake(self, settings, clientHello, serverHello):
        ...
        if cl_kex is None:
            raise TLSIllegalParameterException("Server selected not advertised"
                                               " group.")
        if sr_kex.group in GroupName.allFF:
            kex = FFDHKeyExchange(sr_kex.group, self.version)
        else:
            kex = ECDHKeyExchange(sr_kex.group, self.version)

        Z = kex.calc_shared_key(cl_kex.private, sr_kex.key_exchange)
        ...
        signature_context = bytearray(b'\x20' * 64 +
                                      ...
                                      b'\x00') + \
                            srv_cert_verify_hh.digest(prf_name)
        ...
        verify_data = secureHMAC(finished_key, transcript_hash, prf_name)
        ...
        cl_verify_data = secureHMAC(cl_finished_key,
                        self._handshake_hash.digest(prf_name), prf_name)
    ...
    def _handshakeServerAsyncHelper(self, verifierDB,
        ...
        # Do not support TLS 1.3 on server side
        if settings.maxVersion > (3, 3):
            settings.maxVersion = (3, 3)
        if (0x7f, 21) in settings.versions:
            settings.versions.remove((0x7f, 21))
